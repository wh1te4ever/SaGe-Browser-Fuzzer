...
git_clone_and_setup "https://github.com/libjxl/libjxl.git" "libjxl" "sudo apt install -y cmake pkg-config libbrotli-dev libgif-dev libjpeg-dev libopenexr-dev libpng-dev libwebp-dev && export CC=clang CXX=clang++ && mkdir build && cd build && cmake -DCMAKE_BUILD_TYPE=Release -DBUILD_TESTING=OFF .. && cmake --build . -- -j\$(nproc) && sudo cmake --install ." "Setting up libjxl"
...
# Set compiler to Clang v19.1.7
export CC=/usr/local/bin/clang
export CXX=/usr/local/bin/clang++

export CFLAGS="-DNDEBUG -fsanitize=address -fno-sanitize=alignment -g -w -Wfatal-error"
export CXXFLAGS="-DNDEBUG -fsanitize=address -fno-sanitize=alignment -g -w -Wfatal-error"
export LDFLAGS="-DNDEBUG -fsanitize=address -fno-sanitize=alignment -g -w -Wfatal-error"
export ASAN_OPTIONS="detect_leaks=0"

# Build and install WebKitGTK using Ninja as per user-selected options, removing any previous extracted folder
BUILD_COMMAND="rm -rf webkitgtk-${WEBKITGTK_VERSION} && wget https://webkitgtk.org/releases/webkitgtk-${WEBKITGTK_VERSION}.tar.xz && tar xf webkitgtk-${WEBKITGTK_VERSION}.tar.xz && cd webkitgtk-${WEBKITGTK_VERSION} && mkdir build && cd build && cmake -DUSE_SYSTEM_MALLOC=ON -DENABLE_SANITIZERS=address -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_PREFIX=/usr -DCMAKE_SKIP_RPATH=ON -DPORT=GTK -DLIB_INSTALL_DIR=/usr $CONVERTED_OPTIONS -Wno-dev -G Ninja .. && ninja && sudo ninja install"
